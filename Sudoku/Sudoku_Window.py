import pygame as p
import time
from Sudoku.sudoku_solver import solve, valid
...
class Grid:
    def __init__(self, board):
        ...
        self.cubes = [[Cube(self.board[i][j], i, j)
    ...
    def draw(self, win):
        ...
        for i in range(self.rows + 1):
            if i % 3 == 0 and i != 0:
                ...
                thickness = 1
            p.draw.line(win, (0, 0, 0), (0, i * gap), (self.width,
                                                       i * gap), thickness)  # draw horizontal grid
            p.draw.line(win, (0, 0, 0), (i * gap, 0), (i * gap,
                                                       self.height), thickness)  # draw vertical grid
        # draw cubes
        for i in range(ROWS):
            for j in range(COLUMNS):
    ...
    def update_model(self):
        model = [[self.cubes[i][j].value for j in range(
    ...
    while flag:
        for event in p.event.get():
            if event.type == p.QUIT:
                ...
                if check_button.isOver(pos):
                    ...
                    if solution:
                        bd.cubes = [[Cube(solution[i][j], i, j)
                                     for j in range(COLUMNS)] for i in range(ROWS)]
                    else:
                        message_box(
                            'Unsolved', 'Current board does not have a solution')
                else:
        ...
        if bd.selected and key is not None:
            if bd.sketch_mode:
                ...
                if bd.is_done():
                    message_box(
                        'Game over', 'You have solved the Sudoku. Well done!')
                    flag = False
        key = None
        update_win(win)
    GUI()
...
def custom_board():
    ...
    while not start_solve:
        ...
        if key is not None:
            cusbd.place(key)
        key = None
    board = [[cusbd.cubes[i][j].value for j in range(
        COLUMNS)] for i in range(ROWS)]
    main(board)
